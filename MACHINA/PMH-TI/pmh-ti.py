...
that minimizes migration number mu and then comigration number gamma.
...
- For each candidate resolved tree:
    ...
    - record mu, gamma and choose the candidate with minimal (mu, gamma).
- Output: chosen tree, labeling(s), mu, gamma, sigma, and frequency matrix F.
"""
...
def evaluate_candidate_tree(edges, leaf_labeling_map, sites, pattern_set):
    ...
    and return (tree_obj, labelings, mu_star, gamma_star, sigma_star).
    """
    tree = build_clonetree_from_edges(edges)

    labelings, mu_star, gamma_star, sigma_star = pmh_og.solve_pmh_with_pattern_set(tree, sites, leaf_labeling_map, pattern_set)
    return tree, labelings, mu_star, gamma_star, sigma_star
...
def pmhti_main(tree_path, labels_path, primary_label, pattern_set, outdir):
    ...
    best = None  # tuple: (mu, gamma, candidate_name, tree_obj, labeling, sigma, obj)
    results = []
    for name, edges in candidates:
        try:
            tree_cand, labelings, mu_star, gamma_star, sigma_star = evaluate_candidate_tree(edges, leaf_labeling_map, sites, pattern_set)
        except Exception as e:
            print(f"Candidate {name} raised an error during PMH solving: {e}")
            continue
        for lab in labelings:
            """
            results.append((name, tree_cand, lab, mu_star, gamma_star, sigma_star))
            key = (mu_star, gamma_star)
            if best is None or key < (best[0], best[1]):
                best = (mu_star, gamma_star, name, tree_cand, lab, sigma_star)
            """
            sg = pmh_og.site_graph_from_labeling(tree_cand, lab)
            hub_penalty = compute_hub_penalty(sg, primary_label)

            results.append(
                (name, tree_cand, lab, mu_star, gamma_star, sigma_star, hub_penalty)
            )

            key = (mu_star, gamma_star, sigma_star, hub_penalty)
    ...
    mu_opt, gamma_opt, cand_name, best_tree, best_lab, sigma_opt, hub_penalty_opt = best
    print(f"Best candidate: {cand_name} with mu={mu_opt}, gamma={gamma_opt}, sigma={sigma_opt}")
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(f"gamma_opt\t{gamma_opt}\n")
        f.write(f"sigma_opt\t{sigma_opt}\n")

    lab_path = outdir / "labeling_pmh_ti.txt"
    with open(lab_path, "w") as f:
        f.write(f"# candidate {cand_name}, mu={mu_opt}, gamma={gamma_opt}, sigma={sigma_opt}\n")
